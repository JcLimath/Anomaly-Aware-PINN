...
class Model:
    ...
    def _compile_pytorch(self, lr, loss_fn, decay, loss_weights):
        ...
        def outputs_losses(training, inputs, targets, auxiliary_vars, losses_fn):
            ...
            # Weighted losses
            if loss_weights is not None:
                # print("hhh", loss_weights)
                # losses = loss_weights.reshape(-1, 1).requires_grad_(True) * torch.tensor(losses).reshape(-1,1)
                losses[0] *= loss_weights.reshape(-1, 1).requires_grad_(True)

                # print(losses)
            losses[0] = torch.mean(losses[0])
