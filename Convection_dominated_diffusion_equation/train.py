...
def load_cfg() -> Tuple[Config, str]:
    ...
    model = EquArch(model=model).to('cuda')
    #load(model, '')
...
def visual(
    ...
    for title in titles[:3]:
        ...
        fp.lineplot(x, y1, index=title, marker='', label='Target')
        fp.lineplot(x, y2, index=title, marker='', label='Pred')
    for title in titles[:3]:
        x = running_target['x']
        y1 = running_target[title]
        y2 = running_pred[title]
...
def main(
    ...
    import numpy as np
    np.savetxt('losshistory/losshistory_1e-3_6000_0.8.txt', loss_logger.train.history)
    ...
    data = visual(coach.model, validloader, info_path, log_path, epoch=opts.epochs)
...
def u(x, eps):
    return torch.cos(torch.pi * x / 2) * (1 - torch.exp(-2 * x / eps))
